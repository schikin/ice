...
//	buildMsg := func(class stun.MessageClass, username, key string) *stun.Response {
...
func testBuildAgent(net *vnet.Net) (*Agent, error) {
	lf := logging.NewDefaultLoggerFactory()

	lf.DefaultLogLevel = logging.LogLevelDebug
	lf.ScopeLevels["ice"] = logging.LogLevelDebug

	//url, err := ParseURL("stun:stun.l.google.com:19302")
	url, err := ParseURL("stun:1.2.3.4:3478")

	if err != nil {
		return nil, fmt.Errorf("failed to create STUN URL: %v", err)
	}

	agentConfig := AgentConfig{
		MulticastDNS:              MulticastDNSParams{Mode: MulticastDNSModeDisabled},
		ConnectionTimeout:         nil,
		KeepaliveInterval:         nil,
		NetworkTypes:              nil,
		GatheringTypes:            []GatheringType{GatheringTypeSrflx, GatheringTypeHost},
		LoggerFactory:             lf,
		MaxBindingRequests:        nil,
		CandidateSelectionTimeout: nil,
		...
		TrickleMode:               nil,
		Standard:                  nil,
		STUNConfig:                &STUNConfig{Servers: []*URL{url}},
		TURNConfig:                nil,
		Pacing:                    nil,
		Mode:                      nil,
	}
	...
	if err != nil {
		return nil, fmt.Errorf("failed to create agent: %s", err)
	}

	return agent, nil
}

func TestMutualSTUN(t *testing.T) {
	v, err := buildVNet(&vnet.NATType{
		MappingBehavior:   vnet.EndpointIndependent,
		FilteringBehavior: vnet.EndpointIndependent,
	})
	...
	response, err := agent2.AcceptSession(*proposal)

	if err != nil {
		t.Fatal(err)
	}

	concludeErr := agent1.AcceptResponse(*response)
...
func buildLocalSessionRequest() LocalSessionRequest {
	...
	return LocalSessionRequest{Streams: streamRequests}
}

func TestLocalProposal(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to initiate ICE session: %v", err)
	}
